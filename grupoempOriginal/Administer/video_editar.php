...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	...
	if($_FILES['ima']['name']==""){
		...
		$varimagen = "VIDEO-".$codigounico.$_FILES['ima']['name'];
		}
				
  $updateSQL = sprintf("UPDATE tbl_videos SET titulo=%s, code=%s, contenido=%s, imagen=%s, estado=%s, embed=%s, categoria=%s WHERE id=%s",
  ...
  if($_FILES['ima']['name']!= ""){
		copy($_FILES['ima']['tmp_name'],"../public/VIDEO-".$codigounico.$_FILES['ima']['name']); 
		 $dim = getimagesize("../public/VIDEO-".$codigounico.$_FILES['ima']['name']);
		 
		 if($dim[0]>=500){
				
				
  						$thumb1=new thumbnail("../public/VIDEO-".$codigounico.$_FILES['ima']['name']); 
					    $thumb1->size_width(500);
					    $thumb1->jpeg_quality(90);
					    $thumb1->save("../public/VIDEO-".$codigounico.$_FILES['ima']['name']);
...
mysql_select_db($database_admin, $admin);
$query_editar = sprintf("SELECT * FROM tbl_videos WHERE id = %s", GetSQLValueString($colname_editar, "int"));
					...
					<header class="page-header">
						...
						<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Editar Video</h2>
							</header>
