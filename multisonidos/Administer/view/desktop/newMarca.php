...
<html class="fixed">
    ...
    if(isset($_POST['submit'])){
        ...
        if (empty($nombre) or empty($posicion)  or empty($tag)){
            $param=[
                    'ms'=>'Llene todos los campos con asterísco (obligatorio)',
                    'clase'=>'alert-danger',
                    'alert'=>'Error'
                    ];
            header('Location: newMarca.php?ms='.$param["ms"].'&clase='.$param["clase"].'&alert='.$param["alert"].'');
        }else {
            $marca = new Marca(new Conexion());
            $marca->setNombre($nombre);
            $marca->setTag($tag);
            $marca->setPosicion($posicion);
            $marca->setImg($icono);
            $resul = $marca->save();
            $param=[
            ...
            $mar=$marca->getUltimoRegistro();

           header('Location: editMarca.php?id='.$mar->id);
                                ...
                                <div class="card mb-3 mt-1">
                                    ...
                                    if (empty($ms)){}else{?>
                                        ...
                                        <div class="alert <?php echo $_GET['clase'] ?>">
                                            ...
                                            <a href="marcas.php" class="btn btn-primary pull-right cursor-pointer mr-1" >Finalizar</a>
                                        </div>
                                    <?php }?>
                                    <div class="card-body ">
                                        <form id="loginForm" action="newMarca.php" method="POST"  class="form-horizontal" >
                                            <div class="form-row">
                                                <input type="hidden" name="icono"  value="">
                                               <!-- <div class="col-md-12">
                                                    <div class="form-group iconos">
                                                        <label><span class="text-label">*Icono</span><small> Seleccione un icono para la marca</small></label><br/>
                                                ...
                                                <div class="col-md-6 col-md-offset-3">
                                                    <div class="form-group">
                                                        <label><span class="text-label">*Marca</span></label>
                                                        <input class="form-control" type="text" name="nombre" placeholder="Nombre de la marca">
                                                    </div>
                                                </div>
                                                <div class="col-md-6 col-md-offset-3">
                                                    <div class="form-group">
                                                        <label><span class="text-label">*tag</span></label>
                                                        <input class="form-control" type="text" name="tag" placeholder="Escriba una etiqueta">
                                                    </div>
                                                </div>
                                                <div class="col-md-6 col-md-offset-3">
                                                    <div class="form-group">
