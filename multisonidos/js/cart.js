function pagination(partida, filter, min, max, order, orderold, cat){
    var url = 'MS-paginar-productos.php';
    $.ajax({
        type:'POST',
        url:url,
        data:{
...
function pagination2(partida, filter, order, orderold, idcat){
    var url = 'MS-paginar-productos-categoria.php';
    $.ajax({
        type:'POST',
        url:url,
        data:{
...
jQuery(function ($) {
    $(document).ready(pagination(1, 'pag'));
    $('.buscar').focusout(function(){
        var palabra = $(this).val();
        if(palabra!='') {
            window.location.href = "MS-portafolio_servicios.php?criterio=" + palabra;
        }
    });
    $('.filtervalor').click(function(){
        var min=$('.filtermin').val();
        var max=$('.filtermax').val();
        pagination(1,'valor', min, max, '','', '');
    });
    $('.orderbyold').click(function(){
        var order=$('.by').val();
        var orderold=$('.old').val();
        pagination(1,'order', '', '', order,orderold, '');
    });
    $('.orderbyoldpro').click(function(){
        var order=$('.by').val();
        var orderold=$('.old').val();
        var idcat=$(this).data('id');
        pagination2(1,'orderpro', order,orderold, idcat);
    });
    $('.categ').click(function () {
        var cat=$(this).data('id');
        pagination(1,'cat', '', '', '','', cat);
    });
    ...
    $(".agregaItems").on('click','.addCart2',function(){
        $(this).find('.cargando').fadeIn();
        var id =$(this).data('id');
        var valor= $(this).attr('data-valor');
        var name=$(this).data('nombre');
