...
class PageTemplatesHelper {
    ...
    public function get_any_page_id() {
        ...
        if (!empty($all_pages))
            return $this->sample($all_pages)->ID;
        return false;
    }
    ...
    public function get_sample_page_by_slug($slug) {
        ...
        if (empty($pages))
            return false;
        return get_permalink($this->sample($pages)->ID);
    }
    ...
    public function get_sample_author_by_id($id) {
        ...
        if (empty($posts))
            return false;
        return get_permalink($this->sample($posts)->ID);
    }
    ...
    private function _get_sample_post($filename, $post_meta_value) {
        ...
        if (!empty($posts)) {
            return add_query_arg(array('custom_page' => $filename), get_permalink($this->sample($posts)->ID));
        }
    ...
    private function _get_sample_page($filename) {
        ...
        $url = '';
        if (!empty($pages)) {
            $url = get_permalink($this->sample($pages)->ID);
        }
