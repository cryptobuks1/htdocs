 ...
 * @since      1.0.0
 */
class iMedical_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-imedical-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'imedical' )
		);

		// Create the widget.
		$this->WP_Widget(
			'imedical-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'imedical' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'imedical_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'imedical_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'imedical_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'imedical' ),
			'limit'     => 5,
			'show_date' => false
		);
