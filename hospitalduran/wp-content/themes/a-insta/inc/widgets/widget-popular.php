 ...
 * @since      1.0.0
 */
class iMedical_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-imedical-popular posts-thumbnail-widget',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'imedical' )
		);

		// Create the widget.
		$this->WP_Widget(
			'imedical-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts Thumbnails', 'imedical' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'imedical_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'imedical_popular_widget_' . $this->id, $popular );
		...
		if ( $popular->have_posts() ) {
			echo '<ul>';
				...
				while ( $popular->have_posts() ) :
					...
					echo '<li>';
						echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . get_the_post_thumbnail( get_the_ID(), 'imedical-widget-thumb', array( 'class' => 'entry-thumb', 'alt' => esc_attr( get_the_title() ) ) ) . '</a>';
	...
	function flush_widget_transient() {
		delete_transient( 'imedical_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'imedical' ),
			'limit'     => 5,
			'show_date' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_date' ); ?>">
				<?php _e( 'Display post date?', 'imedical' ); ?>
			</label>
		</p>
