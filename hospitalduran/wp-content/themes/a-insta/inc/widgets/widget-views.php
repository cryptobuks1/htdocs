 ...
 * @since      1.0.0
 */
class iMedical_Views_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-imedical-views posts-thumbnail-widget',
			'description' => __( 'Display the most views posts with thumbnails.', 'imedical' )
		);

		// Create the widget.
		$this->WP_Widget(
			'imedical-views',                                        // $this->id_base
			__( '&raquo; Most Views Posts Thumbnails', 'imedical' ), // $this->name
					...
					echo '<li>';
						...
						echo '<div class="entry-info">';
							...
							if ( $instance['show_view'] ) :
								echo '<span class="entry-view">- ' .  number_format_i18n( $view ) . __( ' Views', 'imedical' ) . '</span>';
							endif;
						echo '</div>';
					echo '</li>';
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Views Posts', 'imedical' ),
			'limit'     => 5,
			'show_date' => false,
			'show_view' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_view' ); ?>">
				<?php _e( 'Display post view?', 'imedical' ); ?>
			</label>
		</p>
