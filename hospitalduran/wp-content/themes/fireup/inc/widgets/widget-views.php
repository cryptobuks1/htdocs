 ...
 * @since      1.0.0
 */
class FireUp_Views_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-fireup-views posts-thumbnail-widget',
			'description' => __( 'Display the most views posts with thumbnails.', 'fireup' )
		);

		// Create the widget.
		$this->WP_Widget(
			'fireup-views',                                        // $this->id_base
			__( '&raquo; Most Views Posts Thumbnails', 'fireup' ), // $this->name
					...
					echo '<li>';
						...
						echo '<div class="entry-info">';
							...
							if ( $instance['show_view'] ) :
								echo '<span class="entry-view">- ' . $view . ' ' . __( 'Views', 'fireup' ) . '</span>';
							endif;
						echo '</div>';
					echo '</li>';
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Views Posts', 'fireup' ),
			'limit'     => 5,
			'show_date' => false,
			'show_view' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_view' ); ?>">
				<?php _e( 'Display post view?', 'fireup' ); ?>
			</label>
		</p>
