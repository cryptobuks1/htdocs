 ...
 * @since      1.0.0
 */
class FireUp_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-fireup-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'fireup' )
		);

		// Create the widget.
		$this->WP_Widget(
			'fireup-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'fireup' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'fireup_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'fireup_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'fireup_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'fireup' ),
			'limit'     => 5,
			'show_date' => false
		);
