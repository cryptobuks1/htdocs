 ...
 * @return array
 */
function fireup_entry_markup( $classes ) {
...
add_filter( 'post_class', 'fireup_entry_markup' );
 ...
 * @return string
 */
function fireup_comment_reply_link_filter( $link ) {
	return preg_replace( '/(<a\s)/i', '$1itemprop="replyToUrl"', $link );
}
add_filter( 'comment_reply_link', 'fireup_comment_reply_link_filter', 5 );
 ...
 * @return string
 */
function fireup_comments_popup_link_attributes( $attr ) {
	return ' itemprop="discussionURL"';
}
add_filter( 'comments_popup_link_attributes', 'fireup_comments_popup_link_attributes', 5 );
 ...
 * @return string
 */
function fireup_get_avatar( $avatar ) {
	return preg_replace( '/(<img.*?)(\/>)/i', '$1itemprop="image" $2', $avatar );
}
add_filter( 'get_avatar', 'fireup_get_avatar', 5 );
 ...
 * @return array
 */
function fireup_img_attr( $attr ) {
    $attr['itemprop'] = 'image';
    return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'fireup_img_attr', 10, 2 );